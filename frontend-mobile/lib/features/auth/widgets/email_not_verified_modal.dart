...
class EmailNotVerifiedPromptModal extends StatefulWidget {
  ...
  const EmailNotVerifiedPromptModal({super.key, required this.email});

  @override
  State<EmailNotVerifiedPromptModal> createState() =>
      _EmailNotVerifiedPromptModalState();
}

class _EmailNotVerifiedPromptModalState
  ...
  Future<void> _sendVerificationEmail() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _notificationService.showToast('Verification.Prompt.EmailSent'.tr());
        _startCountdown();
      }
    } catch (e) {
      print('[ERROR] Send verification failed: $e');
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: _emailSent
                  ...
                  : AppColors.colorAccentDanger.withOpacity(0.15),
          ...
          Text(
            _emailSent
                ? 'Verification.Prompt.TitleSent'.tr()
                : 'Verification.Prompt.Title'.tr(),
          ...
          Text(
            _emailSent
                ? 'Verification.Prompt.DescriptionSent'.tr()
                : 'Verification.Prompt.Description'.tr(),
            style: const TextStyle(
              fontSize: 15,
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text(
                        'Verification.Prompt.DidntReceive'.tr(),
                        style: const TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.w600,
                          color: AppColors.colorTextPrimary,
                  ...
                  const SizedBox(height: 12),
                  _buildCheckItem('Verification.Prompt.CheckSpam'.tr()),
                  _buildCheckItem('Verification.Prompt.WaitMinutes'.tr()),
                  _buildCheckItem('Verification.Prompt.CheckEmailCorrect'.tr()),
                ],
              ),
            ),
            const SizedBox(height: 24),
            ...
            child: ElevatedButton(
              ...
              child: _isSending
                  ...
                  : Text(
                      _emailSent
                          ? (_canResend
                                ? 'Verification.Prompt.ResendEmail'.tr()
                                : 'Verification.Prompt.ResendCountdown'.tr(
                                    args: ['$_countdown'],
                                  ))
                          : 'Verification.Prompt.SendEmail'.tr(),
                      style: TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.w600,
          ...
          SizedBox(
            ...
            child: TextButton(
              ...
              child: Text(
                _emailSent
                    ? 'Verification.Prompt.Close'.tr()
                    : 'Verification.Prompt.Cancel'.tr(),
                style: const TextStyle(
                  fontSize: 15,
