    ...
    : IRequestHandler<GetRecentWorkoutsQuery, Result<IReadOnlyList<RecentWorkoutResponse>>>
{
    ...
    public async Task<Result<IReadOnlyList<RecentWorkoutResponse>>> Handle(
        GetRecentWorkoutsQuery request,
        CancellationToken cancellationToken)
    {
        logger.LogDebug(
            "Starting GetRecentWorkouts process for UserId: {UserId} with Take: {Take}",
            request.UserId,
            request.Take);
        ...
        logger.LogInformation(
            "GetRecentWorkouts process completed successfully for UserId: {UserId}. Workouts found: {Count}",
            request.UserId,
            response.Count);
