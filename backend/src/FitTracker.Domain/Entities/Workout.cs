...
public class Workout : BaseEntity
{
        ...
        string? notes = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(name) || name.Length < NameMinLength || name.Length > NameMaxLength)
        {
            throw new ArgumentOutOfRangeException(
                nameof(name),
                name?.Length ?? 0,
                $"Name length must be between {NameMinLength} and {NameMaxLength} characters.");
        }
    ...
    public void SetDuration(TimeSpan duration)
    {
        if (duration.TotalHours > MaxDurationHours)
        {
            throw new ArgumentOutOfRangeException(
                nameof(duration),
                duration.TotalHours,
                $"Duration cannot exceed {MaxDurationHours} hours.");
        }
    ...
    public bool IsToday()
    {
        return WorkoutDate.Date == DateTime.UtcNow.Date;
    }
    ...
    {
        return IsInProgress || Duration.TotalHours > 0;
    }
}