...
public sealed class JwtTokenGenerator(IOptions<JwtSettings> options) : IJwtTokenGenerator
{
    ...
    public string GenerateVerificationToken(Guid userId)
    {
        return GeneratePurposeToken(
            userId,
            TokenPurposes.EmailVerification,
            settings.EmailVerificationTokenValidityInMinutes);
    }

    /// <inheritdoc />
    public string GeneratePasswordResetToken(Guid userId)
    {
        return GeneratePurposeToken(userId, TokenPurposes.PasswordReset, settings.PasswordResetTokenValidityInMinutes);
    }
