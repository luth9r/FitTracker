...
namespace FitTracker.Domain.ValueObjects
{
    ...
    public sealed class Weight : IEquatable<Weight>, IComparable<Weight>
    {
        ...
        public override string ToString() => $"{ValueInKg.ToString("F2", CultureInfo.InvariantCulture)} kg";
        ...
        public string ToString(string unit)
        {
            return unit switch
            {
                KilogramUnit => $"{ValueInKg.ToString("F2", CultureInfo.InvariantCulture)} kg",
                PoundUnit => $"{ToPounds().ToString("F2", CultureInfo.InvariantCulture)} lbs",
                _ => ToString()
            };
        }
    }
