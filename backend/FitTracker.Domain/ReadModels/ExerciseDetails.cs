...
{
    /// <summary>
    /// Represents a comprehensive read model for exercise details, including metadata, personal records (PRs), cumulative statistics, and historical performance data <see cref="ExerciseHistoryPoint"/>.
    /// </summary>
    ...
    /// <param name="IsCustom">Indicates if the exercise was created by a user (<c>true</c>) or is a system default (<c>false</c>).</param>
    ...
    public sealed record ExerciseDetails(
        ...
        int TotalSets = 0,
        int TotalReps = 0,
        double TotalLifted = 0,
        double AvgWeightPerSet = 0,
        double AvgRepsPerSet = 0,
        DateTime? LastPerformed = null,
        IReadOnlyList<ExerciseHistoryPoint> VolumeHistory = default!);
}
